...
只有当用户明确说“需要 `<目标金额>` 元的发票”时，才使用 `scripts/batch_invoices.py`。一次指令只产生一份组合，不按月份重复生成。

1. 扫描归档根目录内所有 `YYYY/MM` 直属规范命名 PDF；允许跨月、跨年选择，但不读取根目录已有的总额文件夹。
2. 排除低于 `10.00` 元的旧文件。
3. 在全部合格发票中选择总额不低于目标且数学上最接近目标的精确组合，不设超额上限；多个组合总额相同时优先日期较早的发票。
4. 全部合格发票总额仍不足目标时，选择全部合格发票，并明确报告 `is_sufficient: false`。
5. 必须先运行 `plan` 并向用户报告选择清单与实际总额；得到本次执行授权后才运行 `apply`。
6. 在归档根目录创建唯一的 `<实际总额>¥` 文件夹，移动选中的 PDF 并保持原文件名。禁止为每个月分别创建一份组合。
7. 使用批次目录内的隐藏 manifest 记录每张发票的原年月路径、内容哈希和写后核验信息。移动失败必须自动回滚；禁止覆盖已有总额文件夹或 PDF。
...
需要撤销时执行 `rollback --batch-dir '<归档根目录>/<实际总额>¥'`。不得把用户举例或规则说明中的金额当作本次执行指令。
